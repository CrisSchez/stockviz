...
schemaCustomer = StructType(    [
      ...
      StructField("r_quantile", IntegerType(), True),
      StructField("f_quantile", IntegerType(), True),
      StructField("m_quantile", IntegerType(), True),
      StructField("RFMScore", DoubleType(), True)

    ]
)
  
rfm_table_score=spark.createDataFrame(rfm_table4,schema=schemaCustomer) 
rfm_table_score.write.format("iceberg").mode("overwrite").saveAsTable('default.rfm_score')
